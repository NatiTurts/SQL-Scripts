...
from .dbo._bvARTransactionsFull P 
LEFT JOIN .dbo._bvARAccountsFull C on C.DCLink = P.AccountLink 
LEFT JOIN .dbo.TrCodes TR on TR.IdTrCodes = P.TrCodeID
...
LEFT JOIN .dbo._bvARAccountsFull C on C.DCLink = P.DrCrAccount 
LEFT JOIN .dbo._bvStockFull S on S.StockLink = P.AccountLink 
LEFT JOIN .dbo._btblBINLocation B on B.idBinLocation = S.iBinLocationID 
LEFT JOIN .dbo._bvARAuditLinkedAccRep PAR on PAR.cAuditNumber = P.cAuditNumber 
LEFT JOIN .dbo.TrCodes TR on TR.IdTrCodes = P.TrCodeID
...
from .dbo._bvGLTransactionsFull P 
LEFT JOIN .dbo.Accounts A on A.Accountlink = P.AccountLink 
LEFT JOIN .dbo._bvARAccountsFull C on C.DCLink = P.DrCrAccount 
LEFT JOIN .dbo._bvARAuditLinkedAccRep PAR on PAR.cAuditNumber = P.cAuditNumber 
LEFT JOIN .dbo.TrCodes TR on TR.IdTrCodes = P.TrCodeID
