...
sum([Total(INV-CRN)]) as [Total(INV-CRN)],
ConsumptionMonths,
round(sum(AveConsumption),0) as AveConsumption,
PlanningPeriod,
--sum([Stock Required (Planned Period)]) as [Stock Required (Planned Period)],
round(Sum(AveConsumption)*[PlanningPeriod],0) as [Stock Required (Planned Period)],
round(case when Sum(AveConsumption)*[PlanningPeriod]+sum(committed)-sum((QuantityonHand+WIP)) < 0 then 0 else Sum(AveConsumption)*[PlanningPeriod]+sum(committed)-sum((QuantityonHand+WIP)) end,0) as [ToOrder(Recommended)]
FROM #Temp
Group By StockCode, StockDescription, ConsumptionMonths, PlanningPeriod
drop table #Temp
