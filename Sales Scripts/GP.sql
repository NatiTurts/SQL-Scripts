...
from ICC_NEW.dbo._bvARTransactionsFull P 
LEFT JOIN ICC_NEW.dbo._bvARAccountsFull C on C.DCLink = P.AccountLink 
LEFT JOIN ICC_NEW.dbo.TrCodes TR on TR.IdTrCodes = P.TrCodeID
...
LEFT JOIN ICC_NEW.dbo._btblBINLocation B on B.idBinLocation = S.iBinLocationID 
LEFT JOIN ICC_NEW.dbo._bvARAuditLinkedAccRep PAR on PAR.cAuditNumber = P.cAuditNumber 
LEFT JOIN ICC_NEW.dbo.TrCodes TR on TR.IdTrCodes = P.TrCodeID
...
from ICC_NEW.dbo._bvGLTransactionsFull P 
LEFT JOIN ICC_NEW.dbo.Accounts A on A.Accountlink = P.AccountLink 
LEFT JOIN ICC_NEW.dbo._bvARAccountsFull C on C.DCLink = P.DrCrAccount 
LEFT JOIN ICC_NEW.dbo._bvARAuditLinkedAccRep PAR on PAR.cAuditNumber = P.cAuditNumber 
LEFT JOIN ICC_NEW.dbo.TrCodes TR on TR.IdTrCodes = P.TrCodeID
