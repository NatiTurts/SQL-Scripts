 ...
 Declare @DateEnd@ date 	= '2018-11-07'
...
round(sum([Total(INV-CRN)])/case when (datediff(month,min(TxDate),dateadd(day,1,@DateEnd@))-case when day(min(TxDate))=1 then 1 else 1 end) =0 then 1 else (datediff(month,min(TxDate),dateadd(day,1,@DateEnd@))-case when day(min(TxDate))=1 then 1 else 1 end) end,0) as AveConsumption,
PlanningPeriod,
